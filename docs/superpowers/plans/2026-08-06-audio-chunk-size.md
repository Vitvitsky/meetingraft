...
```rust
    ...
    ///
    /// Файл пишется не на каждый вызов: `Some` возвращается только когда
    /// этим вызовом пачка ушла на диск. Если один вызов вызвал два
    /// сброса — по разрыву и следом по целевому размеру, — возвращается
