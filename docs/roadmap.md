...
Maps to: Epic 7. Status: **оба источника готовы, очередь ждёт чисел**
(2026-08-23).
...
**Источников у петли два** (2026-08-23). Второй — модель, читающая всю
расшифровку и предлагающая пары «как распознано → чем это было». Она
видит то, чего не видит декодер с окном в тридцать секунд, и ошибается
там, где ставки выше всего: на редком верном слове, которое тянет к
частому соседу. Поэтому прибор `fix-probe` меряет **долю порч**, а не
долю верных, а одобрять пару можно только послушав. Экран у обоих
источников общий, и ждут они одних и тех же чисел. Спека и план —
`docs/superpowers/specs/2026-08-23-llm-term-corrections-design.md`,
...
- **Silero VAD вместо гейта по энергии.** ADR-005 обещает его дважды, а
  живой путь работает на `stt::NoiseGate` — гейте по превышению над
  фоном комнаты. Разница не косметическая: гейт не отличает речь от
  стука по столу, зато решает мгновенно, а VAD отличает, но
  запаздывает.

  С 2026-08-27 Silero стоит второй колонкой в `gate-probe` (модель
  0.6 МБ, движок и так линкуется ради разделения голосов). На синтетике
  запаздывания не нашлось: при известном начале речи оба открылись на
  одном кадре.

  Открывается по трём числам с Мака, и все три — из `docs/mac-verification.md`:
  запуски модели на молчащей комнате, потери речи на настоящей встрече
  (слушать места расхождения) и **ватты**. Последнее решающее: «трогать
  гейт без ваттметра незачем» записано ещё в Epic 18.
