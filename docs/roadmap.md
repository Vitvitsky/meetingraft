...
Maps to: Epic 9. Status: **ядро и FFI готовы (T1–T3), проверены; UI и
артефакты в работе (T4–T6)**.
...
Maps to: новый Epic 12. Status: **planned**.
...
- **Parakeet как второй on-device engine.** Живёт за существующим
  `SttEngine`, поэтому не требует фазы. Открытый вопрос — русский:
  англоязычные версии его не покрывают, многоязычные заявляют европейские
  языки, но качество на русском надо мерить против Whisper large-v3 на
  своих записях. До замера — эксперимент, не план.
