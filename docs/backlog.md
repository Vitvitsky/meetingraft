...
- Снять базу: `powermetrics` по процессу, доля GPU/ANE, вклад каждого
  из перечисленных источников по отдельности
...
Не хватает: FFI, отдающего кусок PCM по диапазону времени и каналу.

### Работы

- Отдача фрагмента аудио сегмента и его проигрывание
- Правка текста сегмента с сохранением исходного распознавания:
  переписанный сегмент должен помнить, что был поправлен вручную — как
  `speaker_pinned` для спикеров (ADR-012), иначе пересбор затрёт правку
- Из правки — предложение термина: surface и canonical заполнены,
  человек только подтверждает
- Область по умолчанию — встреча; повтор той же правки в другой встрече
  предлагает перенос в глобальную
- Применение нового термина к остальным вхождениям в этой же встрече:
  исправив «интра.ру» один раз, человек ждёт, что исправились все

### Найденный дефект схемы

Одна форма обслуживает два разных намерения: «исправляй вот так» и
«просто знай такое слово». Второе выражается тем, что одно и то же
пишут в оба поля — замена пустая, но слово попадает в подсказку Whisper.
Работает, но догадаться невозможно. Нужен либо явный вид записи
«подсказка», либо необязательный canonical.
...
- Порог энергии подобран на глаз (`ENERGY_THRESHOLD = 450`) — измерить
  на реальных записях: это первичная причина, фильтр лечит следствие
