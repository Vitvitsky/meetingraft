...
/// Запас перед найденной задержкой, мс.
///
/// Фильтр смотрит только **назад** от `index - delay`, поэтому оценка,
/// промахнувшаяся хоть на отсчёт в большую сторону, выносит источник эха
/// за край окна: моделировать его нечем, остаток равен сигналу, и
/// `explained` падает до нуля.
///
/// Замерено прибором `echo-probe` 2026-08-09: на той же синтетике оценка
/// давала 1201 отсчёт при истинных 1200 на записях в 4–6 секунд и 1202 на
/// 8–20 секундах. Второй случай — **ноль помеченных окон из 26** там, где
/// первый давал 19 из 20. Разница в один отсчёт, 0.06 мс.
///
/// Запас сдвигает опорное окно вперёд по источнику, и промах в пределах
/// запаса фильтр покрывает сам. Цена — те же отсчёты из `RESPONSE_TAPS`,
/// то есть 3 мс из 64; на ранние отражения, ради которых окно и брали,
/// остаётся 61 мс.
const DELAY_GUARD_MS: u64 = 3;
...
pub fn detect_echo(mic: &[i16], system: &[i16], sample_rate: u32) -> EchoReport {
    ...
    // Не сама оценка, а оценка минус запас: см. `DELAY_GUARD_MS`.
    let aligned = delay.saturating_sub(frames_for(DELAY_GUARD_MS, sample_rate));
    ...
    for (index, (mic_energy, error_energy, system_energy)) in residual.iter().enumerate() {
        ...
        windows.push(EchoWindow {
            ...
            is_echo: marks_echo(mic_level, system_level, explained),
        });
    }
...
/// Считать ли окно эхом.
///
/// Отдельной функцией, потому что правило проверяется перебором, а не
/// прогоном по записи. Внутри `detect_echo` его пришлось бы проверять
/// синтетикой — а она, как выяснилось, не воспроизводит именно тот
/// случай, ради которого условие про опору и заведено.
///
/// **Эхом не бывает то, чему нечего отражать.** Условие про опору — не
/// осторожность, а определение: при молчащем системном канале в микрофоне
/// звучит что угодно, только не эхо.
///
/// Замер 2026-08-14 показал, зачем записывать это явно. На настоящей
/// встрече `explained` вёл себя наоборот: медиана 0.54 там, где опора
/// молчала (902 окна из 3556), и 0.00 там, где она звучала. Механизм не
/// найден — синтетика не воспроизводит его ни на одном уровне тишины,
/// вплоть до RMS 0.6, — но 553 окна с молчащей опорой уже помечались
/// эхом. Каждое такое — выброшенная реплика владельца, самая дорогая из
/// возможных ошибок (ADR-014).
fn marks_echo(mic_rms: f32, system_rms: f32, explained: f32) -> bool {
    mic_rms >= MIN_WINDOW_RMS && system_rms >= MIN_WINDOW_RMS && explained >= ECHO_EXPLAINED
}
...
mod tests {
    ...
    /// Молчащая опора отменяет пометку, каким бы высоким ни был
    /// `explained`.
    ///
    /// Перебором, а не прогоном: синтетика этот случай не производит —
    /// в ней `explained` при тихой опоре честно падает до нуля, и тест
    /// прошёл бы, не проверив ничего. На настоящей записи он вёл себя
    /// наоборот, и 553 окна с опорой RMS 1 помечались эхом.
    ///
    /// Заведомо положительный случай стоит первым: при звучащей опоре
    /// то же самое окно пометкой быть обязано, иначе правило запрещало бы
    /// всё подряд.
    #[test]
    fn a_silent_reference_cancels_the_mark_however_high_the_share() {
        let loud_mic = MIN_WINDOW_RMS * 10.0;
        let loud_ref = MIN_WINDOW_RMS * 10.0;

        assert!(
            marks_echo(loud_mic, loud_ref, 0.95),
            "громкая пара с высокой долей обязана помечаться"
        );
        assert!(
            !marks_echo(loud_mic, MIN_WINDOW_RMS - 1.0, 0.95),
            "окно с молчащей опорой помечено эхом — отражать было нечего"
        );
        assert!(
            !marks_echo(loud_mic, 1.0, 1.00),
            "доля 1.00 при опоре RMS 1 — это не эхо, а испорченное число"
        );
        assert!(
            !marks_echo(MIN_WINDOW_RMS - 1.0, loud_ref, 0.95),
            "тишина в микрофоне эхом не бывает"
        );
        assert!(
            !marks_echo(loud_mic, loud_ref, ECHO_EXPLAINED - 0.01),
            "порог доли обязан продолжать действовать"
        );
    }
    ...
    /// Решение по окну не зависит от длины записи.
    ///
    /// Найдено прибором `echo-probe` 2026-08-09, и иначе не нашлось бы:
    /// все тесты здесь брали шесть секунд, а встреча идёт минуты. На
    /// восьми секундах оценка задержки промахивалась на **один отсчёт** в
    /// большую сторону, источник эха уезжал за край опорного окна, и то
    /// же самое чистое эхо давало ноль помеченных окон из 26 вместо 19 из
    /// 20. Запас `DELAY_GUARD_MS` эту хрупкость и снимает.
    ///
    /// Длины подобраны не наугад: 6 секунд — прежний рабочий случай,
    /// 10 — тот, на котором детектор молчал.
    #[test]
    fn marking_does_not_depend_on_track_length() {
        for seconds in [6usize, 10] {
            let system = speechlike(RATE as usize * seconds, 1);
            let mic = echo_of(&system, 1_200, 0.4);
            ...
            assert!(
                ...
                "{seconds} с: окон нет — проверять нечего"
            );
            assert!(
                report.echo_windows() * 4 >= report.windows.len() * 3,
                "{seconds} с: помечено {} окон из {} — эхо потеряно на длине записи",
                report.echo_windows(),
                report.windows.len()
            );
        }
