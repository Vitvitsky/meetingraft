...
/// Уезжает ли сдвиг от окна к окну.
///
/// Дрейф часов — это когда сдвиг **уходит**, а не когда дрожит на
/// округлении. Разброс в пределах пары шагов огибающей дрейфом не
/// считается: на настоящей встрече сдвиг стоял −1150 мс с тремя окнами на
/// −1200, и первая версия проверки объявила это расхождением часов,
/// потому что считала равенство за рост.
fn looks_like_drift(lags: &[i64]) -> bool {
    if lags.len() < 3 {
        return false;
    }
    let low = lags.iter().copied().min().unwrap_or(0);
    let high = lags.iter().copied().max().unwrap_or(0);
    if high - low <= ENVELOPE_MS as i64 * 2 {
        return false;
    }
    let growing = lags.windows(2).filter(|pair| pair[1] > pair[0]).count();
    let falling = lags.windows(2).filter(|pair| pair[1] < pair[0]).count();
    growing >= lags.len() / 2 || falling >= lags.len() / 2
}
...
fn report_envelope_match(
    ...
    if windows.len() > 1 {
        ...
        if lags.len() >= 3 {
            let (low, high) = (
                lags.iter().copied().min().unwrap_or(0),
                lags.iter().copied().max().unwrap_or(0),
            );
            // Разброс в один шаг огибающей — это округление, а не ход
            // часов. Первая версия считала «выросло или осталось прежним»
            // и на постоянном сдвиге −1150 мс с одним окном −1200
            // объявляла расхождение часов: почти все пары равны, значит
            // почти все «не убывают». Дрейф — это когда сдвиг **уезжает**,
            // а не когда дрожит на округлении.
            let jitter = high - low;
            if looks_like_drift(&lags) {
                println!(
                    "    ! сдвиг уезжает от окна к окну ({low} → {high} мс). Это расходятся\n\
                     \x20     часы двух источников, и одного общего сдвига на всю встречу не\n\
                     \x20     существует"
                );
            } else {
                ...
                println!(
                    "    сдвиг держится около {} мс (разброс {jitter} мс — это округление\n\
                     \x20   шага огибающей, {} мс). Значит смещение постоянное, а не дрейф",
                    sorted[sorted.len() / 2],
                    ENVELOPE_MS
                );
            }
...
mod tests {
    ...
    /// Постоянный сдвиг с дрожанием в один шаг — не дрейф.
    ///
    /// Написан по ложному срабатыванию: на настоящей встрече сдвиг стоял
    /// −1150 мс с тремя окнами на −1200, а прибор объявил расхождение
    /// часов, потому что считал «выросло или осталось прежним». Почти все
    /// пары равны — значит почти все «не убывают».
    #[test]
    fn a_constant_offset_with_one_step_of_jitter_is_not_drift() {
        let lags: Vec<i64> = vec![
            -1150, -1150, -1150, -1150, -1200, -1150, -1150, -1200, -1150, -1150,
        ];

        assert!(!looks_like_drift(&lags), "постоянный сдвиг принят за дрейф");
    }

    /// Настоящий дрейф: сдвиг уезжает на секунды за встречу.
    ///
    /// Нужен рядом с предыдущим: без него правка «не считать дрейфом
    /// ничего» прошла бы за исправление.
    #[test]
    fn a_shift_that_walks_away_is_drift() {
        let lags: Vec<i64> = (0..10).map(|index| -100 - index * 120).collect();

        assert!(
            looks_like_drift(&lags),
            "уезжающий сдвиг не признан дрейфом"
        );
    }
