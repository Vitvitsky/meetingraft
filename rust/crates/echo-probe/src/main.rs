...
///
/// Считается **только по окнам со звучащей опорой**, и это не фильтр для
/// чистоты, а условие осмысленности. Замер 2026-08-14: `explained` даёт
/// медиану 0.54 там, где системный канал молчит, и 0.00 там, где он
/// звучит. Пока эти окна лежали в одной таблице, зазор описывал разницу
/// между «эхо» и «тишина», а не между «эхо» и «своя речь».
fn print_gap(windows: &[&stt::EchoWindow], speech: &LabelledSpeech) {
    let (owner_all, others_all, both) = split_windows(windows, speech);
    ...
    println!(
        "    окон: у владельца {}, у остальных {}, смешанных {both} — они не в счёт",
        owner_all.len(),
        others_all.len()
    );

    // Уровень опоры сам по себе разделяет группы, и это первое, что стоит
    // увидеть: если он разделяет их почти нацело, никакой `explained` для
    // ответа уже не нужен.
    let quiet =
        |group: &[&stt::EchoWindow]| group.iter().filter(|w| w.system_rms < LOUD_RMS).count();
    println!(
        "    с молчащей опорой: у владельца {} из {}, у остальных {} из {}",
        quiet(&owner_all),
        owner_all.len(),
        quiet(&others_all),
        others_all.len()
    ...
    let owner: Vec<&stt::EchoWindow> = owner_all
        .iter()
        .copied()
        .filter(|w| w.system_rms >= LOUD_RMS)
        .collect();
    let others: Vec<&stt::EchoWindow> = others_all
        .iter()
        .copied()
        .filter(|w| w.system_rms >= LOUD_RMS)
        .collect();
    ...
    if owner.is_empty() || others.is_empty() {
        // Одной группы нет — зазора нет по построению. Печатать вместо
        // него медиану второй значило бы выдать половину за целое.
        println!(
            "    зазор не построен: со звучащей опорой у владельца {} окон, у\n\
             \x20   остальных {}. Сравнивать `explained` можно только там, где\n\
             \x20   опора звучала: при молчащей он отвечает не на тот вопрос.\n\
             \x20   Пустая группа владельца означает, что он ни разу не говорил\n\
             \x20   одновременно с собеседниками — про одновременную речь эта\n\
             \x20   запись не скажет ничего",
            owner.len(),
            others.len()
        );
        return;
    ...
    println!(
        "\n    со звучащей опорой:\n\
         \x20     речь владельца: {:>4} окон, explained медиана {:.2}\n\
         \x20     речь остальных: {:>4} окон, explained медиана {:.2}\n\
         \x20     зазор {:+.2}",
        owner.len(),
        owner_median,
        others.len(),
    ...
    // распознанную реплику владельца он не увидит никогда.
