...
fn print_report(report: &EchoReport) {
    if report.windows.is_empty() {
        println!(
            "  задержка не найдена (корреляция {:.2}) — детектор вышел до окон",
            report.delay_correlation
        );
        println!(
            "  это не «эха нет»: сравнивать было не с чем либо сдвиг не опознан.\n\
             \x20 Правдоподобная задержка — единицы и десятки миллисекунд; ноль окон\n\
             \x20 при заведомо звучавшем собеседнике означает отказ, а не чистоту."
        );
        return;
    }
