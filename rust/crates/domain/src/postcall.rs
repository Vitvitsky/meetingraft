...
impl SpeakerSource {
    ...
    pub fn may_overwrite(self, writer: SpeakerSource) -> bool {
        match self {
            ...
            // Канал перебивается всем, включая слепок. Первая редакция
            // ADR-013 запрещала последнее — «слепок уточняет там, где
            // канал промолчал», — и это оказалось правилом, которое не
            // может сработать: пересбор проставляет спикера по каналу
            // **всем** сегментам (`ffi::rebuild`), то есть канал не
            // молчит никогда. Замер 2026-08-14 показал это числами:
            // «подписано 0 · без имени 0» на встрече со 123 подписями и
            // девятью сложенными слепками.
            //
            // Канал — умолчание, проставленное оптом и без единого
            // свидетельства о том, кто говорил. Ручная разметка —
            // свидетельство. Свидетельство уточняет умолчание.
            Self::Channel => true,
...
mod tests {
    ...
    /// Слепок вправе уточнить подпись по каналу.
    ///
    /// Первая редакция ADR-013 это запрещала, и запрет оказался правилом,
    /// которое не может сработать: пересбор проставляет спикера по каналу
    /// всем сегментам, то есть «там, где канал промолчал» не бывает
    /// нигде. На встрече со 123 ручными подписями пересчёт дал
    /// «подписано 0 · без имени 0».
    ///
    /// Ручное при этом по-прежнему неприкосновенно — это проверяет
    /// соседний тест, и вместе они и есть порядок человек → канал →
    /// слепок.
    #[test]
    fn a_voiceprint_may_refine_a_channel_label_but_not_a_human_one() {
