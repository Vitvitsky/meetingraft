...
final class PCMDownmixerTests: XCTestCase {
    ...
    ///
    /// Считается **накопительно по потоку**, а не по одному вызову.
    /// Непрерывный ресемплер придерживает несколько сэмплов внутри и
    /// отдаёт их со следующим чанком, поэтому первый вызов законно короче.
    /// Именно измерение одного вызова когда-то дало вывод «теряется 15%».
    func testDownmixesStereo48kToMono16k() throws {
        ...
        var total = 0
        for _ in 0 ..< 10 {
            total += downmixer.convert(buffer).count
        }

        // Десять чанков по 100 мс = 16 000 кадров на 16 кГц. Допуск
        // покрывает задержку фильтра и не покрывает потерю 15% (2400).
        XCTAssertEqual(Double(total), 16000, accuracy: 256)
    }
