...
enum Theme {
    ...
    /// Текст поверх заливки статусным цветом.
    ///
    /// Противоположен `textPrimary` намеренно, и это не описка. Статусные
    /// цвета выбираются по контрасту со **страницей**: в тёмной теме они
    /// яркие, в светлой — тёмные. Текст на такой заливке спорит уже не со
    /// страницей, а с ней самой, поэтому его светлота идёт в обратную
    /// сторону.
    ///
    /// Пока этого токена не было, `StatusBadge` брал `textPrimary`, и
    /// обе темы читались плохо: белое на жёлтом `#FFD60A` — 1.41:1,
    /// почти чёрное на зелёном `#1D7A32` — 3.11:1. Второе заметил
    /// человек, первое не замечал никто.
    static let textOnStatus = dynamic(light: 0xFFFFFF, dark: 0x1D1D1F)
