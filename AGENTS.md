...
2. **Только русский.** ADR-003 требует ru/en/es, поэтому это второй
   ...
   вещи разом. В post-call движок выбирает `stt::decide_batch_engine` по
   настройке и `policy.primary`; «встречи только по-русски» в модели нет
   (`allowed` всегда `{ru,en,es}`), а латиницы нет **в словаре GigaAM** —
   отсюда и правило, и то, что «авто» включает его лишь при скачанной
   вручную модели.
...
- **Литеральный `%` в локализуемой строке едет в ключ удвоенным.**
  `Text("\(Int(n))%")` даёт ключ `%lld%%`, а не `%lld%`. Вопрос был
  нерешаем без тулчейна, и первая же сборка его решила: `SWIFT_EMIT_LOC_STRINGS`
  дописал в каталог правильные ключи рядом с моими неверными, и
  `check-localization` увидел это как мёртвые ключи без перевода.
- **Xcode дописывает каталог при сборке, и это не поломка.** Извлечённое
  приезжает без переводов, поэтому четвёртая проверка прибора краснеет —
  так и задумано: новый ключ обязан быть переведён. Пустую строку из
  `Picker("", selection:)` он извлекает тоже; ключи без единой буквы
  перевода не требуют.
