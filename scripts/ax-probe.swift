...
//
//  **Разрешение достаётся не скрипту, а тому, кто его запустил.** При
//  `swift scripts/ax-probe.swift` в списке «Универсальный доступ» должен
//  стоять Терминал (или iTerm), а не Swift и не MeetingRaft. Забыть об
...
func selfCheck() -> Bool {
    ...
    if !trusted {
        print("""
          ...
          Разрешения нет. Выдать его надо **тому, кто запускает скрипт**:
          Системные настройки → Конфиденциальность и безопасность →
          Универсальный доступ → добавить Терминал (или iTerm).
          Не Swift и не MeetingRaft: право наследуется от процесса-хозяина.
        """)
        return false
    }
